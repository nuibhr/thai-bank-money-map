...
import {sectionById,definitions,type SectionId} from '@/lib/bank-sections';
...
export default function ResearchPanel({ticker,section,metric,onMetric}:{ticker:string;section:SectionId|null;metric:string|null;onMetric:(s:string)=>void}){
 ...
 <h3>{current?.label||'ข้อมูลธนาคาร'}</h3>
 {current?<div className="metric-list">{current.children.map(([id,label,unit])=><button key={id} className={metric===id?'active':''} onClick={()=>onMetric(id)}><span>{label}</span><strong>{format(data.bank[id])} {data.bank[id]!=null?unit:''}</strong></button>)}</div>:<p>เลือกชั้นในตึกเพื่อดูเงินทุน สินทรัพย์ รายได้ ความเสี่ยง กำไร หรือเงินกองทุน</p>}
 ...
 <form onSubmit={e=>{e.preventDefault();setSearch(query)}}><label htmlFor="research-query">ค้นหลักฐานใน {ticker}</label><div className="search-row"><input id="research-query" maxLength={500} value={query} onChange={e=>setQuery(e.target.value)} placeholder="เช่น NIM, SME, credit cost"/><button type="submit">ค้นหา</button></div></form>
 ...
 <h3>เอกสารอ้างอิง</h3>{data.sources.map(s=><div className="source" key={String(s.id)}>{safeUrl(s.url)?<a href={safeUrl(s.url)!} target="_blank" rel="noreferrer">{s.title} ↗</a>:<span>{s.title}</span>}<small>{s.id} · งวด {s.period||'ไม่ระบุ'} · เผยแพร่ {s.published||'ไม่ระบุ'}</small></div>)}
 </>}
 </div>
}
